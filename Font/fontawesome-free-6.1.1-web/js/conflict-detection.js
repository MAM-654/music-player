...
(function (global, factory) {
  ...
  function detectWebfontConflicts() {
    ...
    var styconstags = Array.from(DOCUMENT.getElementsByTagName('style')).filter(function (t) {
      if (t.hasAttribute(detectionIgnoreAttr)) {
        return false;
      } // If the browser has loaded the FA5 CSS, const's not test that <style> element.
    ...
    for (var _i = 0; _i < styconstags.length; _i++) {
      var styleUnderTest = DOCUMENT.createElement('style');
      styleUnderTest.setAttribute('id', nodeUnderTestId);
      var md5ForStyle = md5ForNode(styconstags[_i]);
      styleUnderTest.setAttribute(md5Attr, md5ForStyle);
      styleUnderTest.innerText = styconstags[_i].innerText;
      cssByMD5[md5ForStyle] = styconstags[_i];
